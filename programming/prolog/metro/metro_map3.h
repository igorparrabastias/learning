R is wagons_in_use :- R:= 66.
R is time_period :- R:=1000. % minimum 1000
R is end_map :- R:= 0.

finish_station(Where, I, Posoka):-
  Where=14, wPosoka(I)=:=Posoka; Where=16, wPosoka(I)=:= -1*Posoka.
...
station(no, 19, _, _, "Opalchenska", "").
station(no, 18, _, _, "Konstantin", "Velichkov").
station(no, 17, _, _, "Vardar", "").
station(yes, 16, 500000,  40, "Zapaden", "Park").
station(yes, 15, 300000,  20, "Lyulin", "").
